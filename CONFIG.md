...
<product>-promo-config/
  ...
  scripts/apply.py              # secrets -> active ~/.claude.json (forked from market-intel-config; never echoes values)
...
#    fork scripts/apply.py from market-intel-config, then confirm it is ready:
...
**not** generate `apply.py`: that single mechanism is forked from `market-intel-config`, keeping one
