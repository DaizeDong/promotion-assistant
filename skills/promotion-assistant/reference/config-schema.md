...
structure from `market-intel-config`'s template; secrets are **always gitignored** (Mode B — promo
...
<product>-promo-config/
  ...
  scripts/apply.py        # secrets -> active ~/.claude.json (never echoes values); forked from market-intel-config
