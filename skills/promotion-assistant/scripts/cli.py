...
def cmd_apply(args):
    ...
    if not applier.is_file():
        print("config repo has no scripts/apply.py (fork it from market-intel-config). Skipping.")
        return 1
...
def cmd_doctor(args):
    ...
    print("relay        :", "OK" if (Path.home() / ".claude/discord_relay/send.py").is_file() else "MISSING")
    print("send-gmail   :", "OK" if (Path.home() / ".claude/scripts/send-gmail.ps1").is_file() else "MISSING")
